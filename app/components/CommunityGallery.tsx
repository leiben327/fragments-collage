...
function floatLoop(reduce: boolean, seed: number) {
...
export function CommunityGallery() {
  ...
  return (
    ...
    >
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.04] mix-blend-multiply [background-image:repeating-linear-gradient(92deg,transparent,transparent_48px,rgba(61,56,50,0.08)_48px,rgba(61,56,50,0.08)_49px)]"
        aria-hidden
      />
      ...
      <div className="relative mx-auto max-w-[min(100%,88rem)]">
        ...
        <motion.p
          ...
          viewport={{ once: true, margin: "-80px" }}
          transition={reduce ? { duration: 0 } : { duration: 1.5, delay: 0.12 }}
        >
...
function FragmentWallCard({
  ...
  const drift = floatLoop(reduce, index);

  return (
    <motion.li
      layout
      className="mb-8 break-inside-avoid sm:mb-10"
      initial={{ opacity: 0, y: 18, filter: reduce ? "none" : "blur(4px)" }}
    ...
    >
      <motion.div className="relative" animate={drift ?? false}>
        <div style={{ transform: `rotate(${rot}deg)` }}>
          <motion.article
            layout
            className="group relative overflow-visible rounded-[3px_6px_4px_2px] border border-ink/10 bg-gradient-to-b from-cream/90 via-paper/88 to-paper-deep/45 p-4 pb-5 shadow-[6px_22px_40px_rgba(61,56,50,0.1)] ring-1 ring-ink/5 sm:p-5"
            whileHover={
              reduce
                ? {}
                : {
          ...
          >
        <div className="pointer-events-none absolute -left-1.5 top-10 z-[1] h-14 w-6 -rotate-[14deg] rounded-[1px] border border-ink/8 bg-paper-deep/35 opacity-55 shadow-sm" />
        <div className="pointer-events-none absolute -right-0.5 bottom-16 z-[1] h-11 w-7 rotate-[12deg] rounded-[1px] border border-ink/8 bg-blush/25 opacity-50" />
